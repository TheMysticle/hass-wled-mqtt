...
class WledPresetSelect(SelectEntity):
    ...
    async def _fetch_presets_once(self) -> None:
        ...
        for key, value in data.items():
            # WLED presets.json keys are numeric strings; key "0" is a placeholder, skip it
            ...
            # If the preset has a segment list with exactly one active segment,
            # record it so we can sync the segment selector later
            segs = value.get("seg")
            if isinstance(segs, list):
                active = [s.get("id", i) for i, s in enumerate(segs) if isinstance(s, dict) and s.get("on", True)]
                if len(active) == 1:
