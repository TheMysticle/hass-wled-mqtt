...
class WledMqttOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            ...
            # Save only the editable fields to entry.options.
            # entry.data is never modified after initial setup — get_config()
            # merges options over data at runtime so options always win.
            return self.async_create_entry(
                title="",
                data={
                    ...
                    CONF_NUM_SEGMENTS: user_input.get(CONF_NUM_SEGMENTS, DEFAULT_NUM_SEGMENTS),
                    CONF_DETECT_PRESETS: user_input.get(CONF_DETECT_PRESETS, DEFAULT_DETECT_PRESETS),
